      ...
      request(opts, function (error, response, body) {
        ...
        if (error) {
          if (error.code === 'ETIMEDOUT') {
            node.error(RED._("no-response"), msg);
            setTimeout(function () {
              node.status({
                fill: "red",
                shape: "ring",
          ...
          // msg.payload = data.map( d=>{ 
    
          // const date = new Date(d.ts)
          // const localDate = date.toLocaleString()
   
  // return img ="http://demo.vamstack.com"+ d.image.cropped
...
// })
  ...
  node.send(msg);
    ...
    RED.nodes.registerType("vamapi", VamgetApiNode )
    // , {  
    //   // credentials: {
    //   //   user: {
    //   //     type: "text"
